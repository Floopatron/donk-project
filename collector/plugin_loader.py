"""
Plugin Loader for Donk Collector

Discovers and loads collector plugins from the plugins directory.
Each plugin must have a manifest.json and collector.py file.
"""
...
class PluginLoader:
    """Manages loading and instantiation of collector plugins"""
    ...
    def load_manifest(self, plugin_name: str) -> Optional[dict]:
        ...
        try:
            ...
            required_fields = ["plugin_id", "version", "name", "collector"]
            ...
            if "file" not in collector_config or "class" not in collector_config:
                logger.error(f"Plugin {plugin_name}: Invalid collector configuration")
                return None
    ...
    def load_plugin_class(self, plugin_name: str, manifest: dict):
        ...
        """
        collector_config = manifest["collector"]
        plugin_file = collector_config["file"]
        class_name = collector_config["class"]

        plugin_path = self.plugins_dir / plugin_name / plugin_file

        if not plugin_path.exists():
            logger.error(f"Plugin {plugin_name}: Collector file not found: {plugin_file}")
            return None

        try:
            # Load module dynamically
            spec = importlib.util.spec_from_file_location(
                f"donk_plugin_{plugin_name}",
                plugin_path
            )
        ...
        for plugin_name in plugin_names:
            try:
                ...
                self.loaded_plugins[plugin_id] = {
                    ...
                    "instance": instance,
                    "name": plugin_name,
                    "update_interval": manifest["collector"].get("update_interval", 30)
                }
